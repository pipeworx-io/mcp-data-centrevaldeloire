 ...
 *
 * Centre-Val de Loire Open Data — open data for the Centre-Val de Loire region, France via the OpenDataSoft Explore v2.1 API: regional services, transport, education & environment. Keyless.
 * Use `search_datasets` (find a dataset_id) -> `dataset_info` (schema) -> `query` (ODSQL records).
 */


const BASE = 'https://data.centrevaldeloire.fr';
const UA = 'pipeworx-mcp-data-centrevaldeloire/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [
  {
    name: 'search_datasets',
    description: 'Search Centre-Val de Loire Open Data for datasets by keyword (regional services, transport, education & environment). Returns dataset_ids (pass to query/dataset_info), titles, themes and record counts.',
  ...
  {
    name: 'dataset_info',
    description: 'Get metadata for one Centre-Val de Loire Open Data dataset (fields/schema, themes, record count) — call before query to learn the column names.',
    inputSchema: { type: 'object', properties: {
  ...
  {
    name: 'query',
    description: 'Query records from a Centre-Val de Loire Open Data dataset with ODSQL. Filter (where), aggregate (group_by/select), sort (order_by), paginate (limit/offset).',
    inputSchema: { type: 'object', properties: {
      dataset_id: { type: 'string', description: 'Dataset id from search_datasets.' },
...
async function get(path: string): Promise<unknown> {
  ...
  if (!res.ok) throw new Error(`Centre-Val de Loire Open Data: ${res.status} ${await res.text().then((t) => t.slice(0, 160))}`);
  return res.json();
}
